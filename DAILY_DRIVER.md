...
Release tag 格式为 `daily-v<上游版本>-<源码 SHA 前 12 位>`，指向真正构建的源码提交。附件包括 VSIX、`SHA256SUMS` 和 `provenance.json`（记录上游、日用源码及自动化提交）。重复运行不会覆盖已发布的 tag 或附件。若上传失败留下不完整的 draft，下一次运行会重新测试构建；确认草稿仍未发布且目标是同一源码提交后，自动重建该草稿并发布。已发布版本或来源不匹配的草稿不会被删除。
